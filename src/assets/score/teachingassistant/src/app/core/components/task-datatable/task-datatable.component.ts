...
export class TaskDatatableComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.periodName = this.upgrade.$injector
            .get('TeacherDataService')
            .getCurrentPeriod().periodName;
        this.upgrade.$injector
            .get('$rootScope')
            .$on('currentPeriodChanged', (event, args) => {
                ...
                .subscribe((tasks) => {
                    ...
                    for (let i = 0; i < tasks.length; i++) {
                        ...
                        if (task.complete == false) {
                            this.tasksDataSource.data.filter(function (
                                element,
                            ) {
    ...
    sendRequestApprovedMessageToStudent(taskRequest: TaskRequest) {
        this.websocketService.sendRequestApprovedMessageToStudent(
            taskRequest.workgroupId,
            taskRequest.runId,
        );
    }
}
